...
class UserController extends Controller
{
    ...
    public function register() {
        ...
        $existingUser = $this->service->checkUsernameEmail($postedUser->username, $postedUser->email);
        if($existingUser) {
            $this->respondWithError(401, "Username or email already taken");
            return;
        }

        // register the user in the db
            $this->service->registerUser($postedUser);

        // generate jwt
        //resond with success message
        $tokenResponse = array(
